...
def krylov_solver(matrix_vector_product, hdiag, problem_type='eigenvalue',
    ...
    Args:
        ...
        gs_initial: bool
            initial guess were already orthonormalized, meaning no need to apply Gram-Schmidt orthogonalization.
        gram_schmidt: bool
    ...
    if problem_type == 'eigenvalue':
        _converged, _energies, X_new = initguess_fn(n_states=size_new, hdiag=hdiag)

    elif problem_type == 'linear':
        _converged, X_new = initguess_fn(hdiag=hdiag, rhs=rhs)
    ...
    if gs_initial:
        ...
        n_new_vectors = normalize_new_vecs(V_holder, 0, X_new)
    ...
    for ii in range(max_iter):
        ...
        log.info(f'     X_new {X_new.shape} {X_new.nbytes//1024**2} MB')
        ...
        mvp = matrix_vector_product(X_new[:n_new_vectors,:])
        # del X
        release_memory()
        ...
        sub_A_holder = math_helper.gen_VW_symmetry(sub_A_holder, V_holder, X_new[:n_new_vectors,:], mvp, size_old, size_new)
        ...
        if not gram_schmidt:
            ...
            math_helper.gen_VW_symmetry(s_holder, V_holder, X_new[:n_new_vectors,:], X_new[:n_new_vectors,:], size_old, size_new)
        ...
        if in_ram:
            X_new = X_new.get()
        V_holder[size_old:size_new, :] = X_new[:n_new_vectors,:]
        del X_new
        size_old = size_new
...
#     if gram_schmidt:
#         log.info('Using Gram-Schmidt orthogonalization')
#         normalize_new_vecs = partial(math_helper.VW_Gram_Schmidt_normalize_new_vecs, double=True)
...
#         '''
#         log.info(nks_citation)
#         normalize_new_vecs = math_helper.VW_nKs_normalize_new_vecs
...
#     else:
#         V_holder, W_holder, size_new = normalize_new_vecs(V_holder=V_holder,
#                                                 W_holder=W_holder,
#                                                 count=size_old,
#                                                 X_new=init_guess_X,
#                                                 Y_new=init_guess_Y)
