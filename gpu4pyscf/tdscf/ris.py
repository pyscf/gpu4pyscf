...
def get_Tpq(mol, auxmol, lower_inv_eri2c, C_p, C_q, 
                                ...
                                group_size=BLKSIZE, group_size_aux=AUXBLKSIZE, log=None, in_ram=True, single=True):
    """
    ...
    if in_ram:
        ...
        contract = einsum2dot 
        lower_inv_eri2c = lower_inv_eri2c.get()

    if calc == 'J':
        Tia = contract('PQ,Qia->Pia',lower_inv_eri2c.T, Pia)
        return Tia

    if calc == 'K':
        Tij = contract('PQ,Qij->Pij',lower_inv_eri2c.T, Pij)
        Tab = contract('PQ,Qab->Pab',lower_inv_eri2c.T, Pab)
        return Tij, Tab
