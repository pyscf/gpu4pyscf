...
from gpu4pyscf.dft import rkspu
from gpu4pyscf.grad import rkspu as rkspu_grad
...
class KnownValues(unittest.TestCase):
    def test_finite_diff_local_orbitals(self):
        ...
        ref = (C0p - C0m) / 2e-3
        self.assertAlmostEqual(abs(C1[1] - ref).max().get(), 0, 6)
        ...
        ref = (C0p - C0m) / 2e-3
        self.assertAlmostEqual(abs(C1[2] - ref).max().get(), 0, 6)
