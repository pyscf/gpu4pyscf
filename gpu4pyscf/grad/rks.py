    ...
    if xctype == 'LDA':
        ...
        for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv, max_memory):
            for idm in range(nset):
                ...
                rho = numint.eval_rho2(_mol, ao_mask[0], mo_coeff_mask, mo_occ, None, xctype)
        ...
        for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv, max_memory):
            for idm in range(nset):
                mo_coeff_mask = mo_coeff[idx,:]
                rho = numint.eval_rho2(_mol, ao_mask[:4], mo_coeff_mask, mo_occ, None, xctype)
                vxc = ni.eval_xc_eff(xc_code, rho, 1, xctype=xctype)[1]
                wv = weight * vxc
                wv[0] *= .5
        ...
        for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv, max_memory):
            for idm in range(nset):
                mo_coeff_mask = mo_coeff[idx,:]
                rho = numint.eval_rho2(_mol, ao_mask[:4], mo_coeff_mask, mo_occ, None, xctype, with_lapl=False)
                vxc = ni.eval_xc_eff(xc_code, rho, 1, xctype=xctype)[1]
                wv = weight * vxc
                wv[0] *= .5
    ...
    with opt.gdft_envs_cache():
        ...
        for atm_id, (coords, weight, weight1) in enumerate(grids_response_cc(grids)):
            ...
            for p0, p1 in lib.prange(0,ngrids,block_size):
                ...
                rho = numint.eval_rho(_mol, ao, dms[0],
