...
class FSSH:
    ...
    def kTDC(self, energy_t:np.ndarray, energy_p:np.ndarray, energy_pp:np.ndarray):
        """
        Kapa time-derivative coupling.
        ...
        for idx in self.nac_idx:
            ...
            sqrt_part = np.where(sqrt_part>0, sqrt_part, 0)
            nact[j, i] = np.sqrt(sqrt_part)
            nact[i, j] = -nact[j, i]
        return nact
    ...
    def calc_electronic(self, position):
        ...
        try:
            ...
            energy = np.concatenate([[self.tddft._scf.e_tot], self.tddft.e_tot.get()])[self.states] # (Ns,)  Unit: Ha
    ...
    def check_hop(self, r:float, g_ij) -> int:
        """
        Check if hop occurs.
    ...
    def rescale_velocity(self, hop_index, energy, velocity, d_vec):
        """
        Rescale velocity.
    ...
    def kernel(self, position, velocity, coefficient):
        ...
        print('start_writing coordinate.xyz')
        with open('coordinate.xyz','w') as f:
            content = f'{len(self.tddft.mol.atom)}\n\n' 
            content += '\n'.join([f'{i[0][0]}  {i[1][0]/A2BOHR:.4f}   {i[1][1]/A2BOHR:.4f}   {i[1][2]/A2BOHR:.4f}' for i in zip(self.tddft.mol.atom,position)]) + '\n'
            f.write(content)        
        ...
        for i in range(self.nsteps):
            ...
            energy, force = self.calc_electronic(position) # TODO check for the direction of NACV
