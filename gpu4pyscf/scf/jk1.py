...
def get_k_o0(mol, dm, hermi=0, vhfopt=None, verbose=None):
    '''Compute K matrix
    '''
    log = logger.new_logger(mol, verbose)
    cput0 = log.init_timer()
    ...
    vk = vhfopt.get_k_o0(dms, hermi, log)
    #:vk = cp.einsum('pi,npq,qj->nij', vhfopt.coeff, vk, vhfopt.coeff)
    vk = vhfopt.apply_coeff_CT_mat_C(vk)
    vk = vk.reshape(dm.shape)
    log.timer('vk', *cput0)
...
class _VHFOpt:
    ...
    def get_k(self, dms, hermi, verbose):
        ...
        def proc(dms, dm_cond):
            ...
            for task in tasks:
                ...
                scheme = _quartets_scheme_for_k(mol.omega, ls)
                gxyz_offset = _make_gxyz_offset(*ls)
                err = kern(
                    ctypes.cast(vk.data.ptr, ctypes.c_void_p),
                    ctypes.cast(dms.data.ptr, ctypes.c_void_p),
                    ctypes.c_int(n_dm), ctypes.c_int(nao),
                    self.rys_envs, (ctypes.c_int*3)(*scheme),
                    (ctypes.c_int*8)(*shls_slice),
                    ...
                    ctypes.cast(gxyz_offset.data.ptr, ctypes.c_void_p),
                    mol._atm.ctypes, ctypes.c_int(mol.natm),
                    mol._bas.ctypes, ctypes.c_int(mol.nbas), mol._env.ctypes)
                if err != 0:
                    llll = f'({l_symb[i]}{l_symb[j]}|{l_symb[k]}{l_symb[l]})'
                ...
                if log.verbose >= logger.DEBUG1:
                    llll = f'({l_symb[i]}{l_symb[j]}|{l_symb[k]}{l_symb[l]})'
                    msg = f'processing {llll} on Device {device_id}'
                    t1, t1p = log.timer_debug1(msg, *t1), t1
                    timing_counter[llll] += t1[1] - t1p[1]
                    kern_counts += 1
                if num_devices > 1:
    ...
    def get_k_o0(self, dms, hermi, verbose):
        '''
        Build JK for the sorted_mol. Density matrices dms and the return K
        matrix are all corresponding to the sorted_mol
        '''
        ...
        def proc(dms, dm_cond):
            ...
            init_constant(mol) # initialize c_g_pair_idx
            libvhf_rys.RYS_build_k_init(ctypes.c_int(SHM_SIZE))
            timing_counter = Counter()
            kern_counts = 0
            kern = libvhf_rys.RYS_build_k_o0
            ...
            for task in tasks:
                ...
                if len(pair_ij_mapping) == 0 or len(pair_kl_mapping) == 0:
                    continue
                ls = uniq_l_ctr[list(task),0]
                scheme = _quartets_scheme_for_k_o0(mol.omega, ls)
                err = kern(
                    ctypes.cast(vk.data.ptr, ctypes.c_void_p),
                    ctypes.cast(dms.data.ptr, ctypes.c_void_p),
                    ctypes.c_int(n_dm), ctypes.c_int(nao),
...
def _quartets_scheme_for_k(omega, ls, shm_size=SHM_SIZE):
    li, lj, lk, ll = ls
    order = li + lj + lk + ll
    nfi = (li + 1) * (li + 2) // 2
    nfj = (lj + 1) * (lj + 2) // 2
    ...
    return nsq_per_block, gout_stride, buflen

def _quartets_scheme_for_k_o0(omega, ls, shm_size=SHM_SIZE):
    li, lj, lk, ll = ls
    order = li + lj + lk + ll
    nfi = (li + 1) * (li + 2) // 2
    nfj = (lj + 1) * (lj + 2) // 2
    ...
    if omega < 0: # SR
        nroots *= 2
    jk_cache_size = (nfi+nfj) * (nfk+nfl)
    root_g_cache_size = nroots*2 + g_size*3 + 13
    unit = max(root_g_cache_size, jk_cache_size)
    counts = shm_size // (unit*8)
    #n = min(THREADS, _nearest_power2(counts))
    #gout_stride = THREADS // n
    #while gout_stride < 16 and gout_size / (gout_stride*GOUT_WIDTH) > 1:
    #    n //= 2
    #    gout_stride *= 2
    #buflen = nsq_per_block * unit*8
    #return n, gout_stride

    gout_stride = min(THREADS, (gout_size + 59) // 60)
    nsq_per_block = min(counts, THREADS // gout_stride)
    buflen = nsq_per_block * unit*8
    return nsq_per_block, gout_stride, buflen

def _make_gxyz_offset(li, lj, lk, ll):
    nfi = (li + 1) * (li + 2) // 2
    nfj = (lj + 1) * (lj + 2) // 2
    nfk = (lk + 1) * (lk + 2) // 2
    nfl = (ll + 1) * (ll + 2) // 2
