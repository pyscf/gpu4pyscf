...
void int3c2e_bdiv_000(double *out, Int3c2eEnvVars envs, BDiv3c2eBounds bounds)
{
    ...
    for (int ijk_idx = st_id; ijk_idx < nst; ijk_idx += nst_per_block) {
        ...
        double *eri_tensor = out_local + k0 + shl_pair_in_block * 1 * naux + ksh_in_block;
...
void int3c2e_bdiv_100(double *out, Int3c2eEnvVars envs, BDiv3c2eBounds bounds)
{
    ...
    for (int ijk_idx = st_id; ijk_idx < nst; ijk_idx += nst_per_block) {
        ...
        int *ao_loc = envs.ao_loc;
        int k0 = ao_loc[ksh0] - ao_loc[nbas];
        double *eri_tensor = out_local + k0 + shl_pair_in_block * 3 * naux + ksh_in_block;
        eri_tensor[0*naux + 0*nksh] = gout0;
...
void int3c2e_bdiv_110(double *out, Int3c2eEnvVars envs, BDiv3c2eBounds bounds)
{
    ...
    for (int ijk_idx = st_id; ijk_idx < nst; ijk_idx += nst_per_block) {
        ...
        int *ao_loc = envs.ao_loc;
        int k0 = ao_loc[ksh0] - ao_loc[nbas];
        double *eri_tensor = out_local + k0 + shl_pair_in_block * 9 * naux + ksh_in_block;
        eri_tensor[0*naux + 0*nksh] = gout0;
        eri_tensor[1*naux + 0*nksh] = gout1;
        eri_tensor[2*naux + 0*nksh] = gout2;
...
void int3c2e_bdiv_200(double *out, Int3c2eEnvVars envs, BDiv3c2eBounds bounds)
{
    ...
    for (int ijk_idx = st_id; ijk_idx < nst; ijk_idx += nst_per_block) {
        ...
        int *ao_loc = envs.ao_loc;
        int k0 = ao_loc[ksh0] - ao_loc[nbas];
        double *eri_tensor = out_local + k0 + shl_pair_in_block * 6 * naux + ksh_in_block;
        eri_tensor[0*naux + 0*nksh] = gout0;
        eri_tensor[1*naux + 0*nksh] = gout1;
        eri_tensor[2*naux + 0*nksh] = gout2;
        eri_tensor[3*naux + 0*nksh] = gout3;
...
void int3c2e_bdiv_210(double *out, Int3c2eEnvVars envs, BDiv3c2eBounds bounds)
{
    ...
    for (int ijk_idx = st_id; ijk_idx < nst; ijk_idx += nst_per_block) {
        ...
        int *ao_loc = envs.ao_loc;
        int k0 = ao_loc[ksh0] - ao_loc[nbas];
        double *eri_tensor = out_local + k0 + shl_pair_in_block * 18 * naux + ksh_in_block;
        eri_tensor[0*naux + 0*nksh] = gout0;
        eri_tensor[1*naux + 0*nksh] = gout1;
        eri_tensor[2*naux + 0*nksh] = gout2;
        eri_tensor[3*naux + 0*nksh] = gout3;
...
void int3c2e_bdiv_220(double *out, Int3c2eEnvVars envs, BDiv3c2eBounds bounds)
{
    ...
    for (int ijk_idx = st_id; ijk_idx < nst; ijk_idx += nst_per_block) {
        ...
        int *ao_loc = envs.ao_loc;
        int k0 = ao_loc[ksh0] - ao_loc[nbas];
        double *eri_tensor = out_local + k0 + shl_pair_in_block * 36 * naux + ksh_in_block;
        eri_tensor[0*naux + 0*nksh] = gout0;
        eri_tensor[1*naux + 0*nksh] = gout1;
        eri_tensor[2*naux + 0*nksh] = gout2;
        eri_tensor[3*naux + 0*nksh] = gout3;
