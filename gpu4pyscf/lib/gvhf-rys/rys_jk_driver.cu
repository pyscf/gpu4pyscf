...
extern __global__ void rys_ejk_ip2_kernel(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds,
                                          ShellQuartet *pool, uint32_t *batch_head);
extern __global__ void rys_ejk_ip2_type1_kernel(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds,
                                          ShellQuartet *pool, uint32_t *batch_head);
extern __global__ void rys_ejk_ip2_type2_kernel(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds,
                                          ShellQuartet *pool, uint32_t *batch_head);
extern __global__ void rys_ejk_ip2_type12_kernel(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds,
                                          ShellQuartet *pool, uint32_t *batch_head);
extern __global__ void rys_ejk_ip2_type3_kernel(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds,
                                          ShellQuartet *pool, uint32_t *batch_head);
extern __global__ void rys_ej_ip2_type3_kernel(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds,
                                          ShellQuartet *pool, uint32_t *batch_head);
...
extern int rys_ejk_ip2_unrolled(RysIntEnvVars *envs, JKMatrix *jk, BoundsInfo *bounds,
                    ShellQuartet *pool, uint32_t *batch_head, int *scheme, int workers);
extern int rys_ejk_ip2_type12_unrolled(RysIntEnvVars *envs, JKMatrix *jk, BoundsInfo *bounds,
                    ShellQuartet *pool, uint32_t *batch_head, int *scheme, int workers);
extern int rys_ejk_ip2_type3_unrolled(RysIntEnvVars *envs, JKMatrix *jk, BoundsInfo *bounds,
                    ShellQuartet *pool, uint32_t *batch_head, int *scheme, int workers);
...
int RYS_per_atom_jk_ip2_type1(double *vj, double *vk, double *dm, int n_dm, int nao,
                        RysIntEnvVars envs, int *scheme, int *shls_slice,
                        int ntile_ij_pairs, int ntile_kl_pairs,
                        int *tile_ij_mapping, int *tile_kl_mapping, float *tile_q_cond,
                        float *q_cond, float *dm_cond, float cutoff,
                        ...
                        int *atm, int natm, int *bas, int nbas, double *env)
{
    ...
    if (1){//!rys_ejk_ip2_unrolled(&envs, &jk, &bounds, pool, batch_head, scheme, workers)) {
        int quartets_per_block = scheme[0];
        int gout_stride = scheme[1];
        int ij_prims = iprim * jprim;
        dim3 threads(quartets_per_block, gout_stride);
        ...
        rys_ejk_ip2_type1_kernel<<<workers, threads, buflen*sizeof(double)>>>(envs, jk, bounds, pool, batch_head);
    }
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Error in RYS_build_jk: %s\n", cudaGetErrorString(err));
...
int RYS_per_atom_jk_ip2_type2(double *vj, double *vk, double *dm, int n_dm, int nao,
                        RysIntEnvVars envs, int *scheme, int *shls_slice,
                        int ntile_ij_pairs, int ntile_kl_pairs,
                        int *tile_ij_mapping, int *tile_kl_mapping, float *tile_q_cond,
                        float *q_cond, float *dm_cond, float cutoff,
                        ...
                        int *atm, int natm, int *bas, int nbas, double *env)
{
    ...
    if (1){//!rys_ejk_ip2_unrolled(&envs, &jk, &bounds, pool, batch_head, scheme, workers)) {
        ...
        int buflen = (nroots*2 + g_size*3 + ij_prims*4) * quartets_per_block;
        buflen = MAX(buflen, 9*gout_stride*quartets_per_block);
        rys_ejk_ip2_type2_kernel<<<workers, threads, buflen*sizeof(double)>>>(envs, jk, bounds, pool, batch_head);
    }
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Error in RYS_build_jk: %s\n", cudaGetErrorString(err));
...
int RYS_per_atom_jk_ip2_type12(double *vj, double *vk, double *dm, int n_dm, int nao,
                        RysIntEnvVars envs, int *scheme, int *shls_slice,
                        int ntile_ij_pairs, int ntile_kl_pairs,
                        int *tile_ij_mapping, int *tile_kl_mapping, float *tile_q_cond,
                        float *q_cond, float *dm_cond, float cutoff,
...
int RYS_per_atom_jk_ip2_type3(double *vj, double *vk, double *dm, int n_dm, int nao,
                        RysIntEnvVars envs, int *scheme, int *shls_slice,
                        int ntile_ij_pairs, int ntile_kl_pairs,
                        int *tile_ij_mapping, int *tile_kl_mapping, float *tile_q_cond,
                        float *q_cond, float *dm_cond, float cutoff,
...
int RYS_per_atom_j_ip2_type3(double *vj, double *vk, double *dm, int n_dm, int nao,
                        RysIntEnvVars envs, int *scheme, int *shls_slice,
                        int ntile_ij_pairs, int ntile_kl_pairs,
                        int *tile_ij_mapping, int *tile_kl_mapping, float *tile_q_cond,
                        float *q_cond, float *dm_cond, float cutoff,
                        ...
                        int *atm, int natm, int *bas, int nbas, double *env)
{
    ...
    if (1) {
        ...
        dim3 threads(quartets_per_block, gout_stride);
        int buflen = (nroots*2 + g_size*3 + ij_prims*4) * quartets_per_block;
        rys_ej_ip2_type3_kernel<<<workers, threads, buflen*sizeof(double)>>>(envs, jk, bounds, pool, batch_head);
    }
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Error in RYS_build_jk: %s\n", cudaGetErrorString(err));
                       ...
                       double *env, int env_size, int shm_size)
{
    ...
    cudaFuncSetAttribute(rys_ejk_ip2_type1_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
    cudaFuncSetAttribute(rys_ejk_ip2_type2_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
    ...
    cudaFuncSetAttribute(rys_ej_ip2_type3_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
}
