                        ...
                        int *atm, int natm, int *bas, int nbas, double *env)
{
    ...
    JKEnergy jk = {ejk, dm, j_factor, k_factor, (uint16_t)n_dm};
                        ...
                        int *atm, int natm, int *bas, int nbas, double *env)
{
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Error in RYS_ejk_ip2: %s\n", cudaGetErrorString(err));
        return 1;
    }
    return 0;
}
...
//int RYS_per_atom_j_ip2_type12(double *vj, double *vk, double *dm, int n_dm, int nao,
//                       RysIntEnvVars envs, int *scheme, int *shls_slice,
//                       int ntile_ij_pairs, int ntile_kl_pairs,
//                       int *tile_ij_mapping, int *tile_kl_mapping, float *tile_q_cond,
//                       float *q_cond, float *dm_cond, float cutoff,
...
//    cudaMemset(batch_head, 0, 2*sizeof(int));
//
//    if (!rys_ej_ip2_type12_unrolled(&envs, &jk, &bounds, pool, batch_head, scheme, workers)) {
//        int quartets_per_block = scheme[0];
//        int gout_stride = scheme[1];
//        int ij_prims = iprim * jprim;
//        dim3 threads(quartets_per_block, gout_stride);
...
//        rys_ej_ip2_type12_kernel<<<workers, threads, buflen*sizeof(double)>>>(envs, jk, bounds, pool, batch_head);
//    }
//    cudaError_t err = cudaGetLastError();
//    if (err != cudaSuccess) {
//        fprintf(stderr, "CUDA Error in RYS_build_jk: %s\n", cudaGetErrorString(err));
...
//    cudaMemset(batch_head, 0, 2*sizeof(int));
//
//    if (!rys_ej_ip2_type3_unrolled(&envs, &jk, &bounds, pool, batch_head, scheme, workers)) {
//        int quartets_per_block = scheme[0];
//        int gout_stride = scheme[1];
//        int ij_prims = iprim * jprim;
//        dim3 threads(quartets_per_block, gout_stride);
