                     ...
                     int *pool, double *dd_pool)
{
    ...
    int smid = get_smid();
    int *bas_kl_idx = pool + smid * QUEUE_DEPTH;
    double *dd_cache = dd_pool + smid * 864 + sq_id;
    __shared__ int ntasks;
    if (sq_id == 0 && gout_id == 0) {
        ntasks = 0;
    }
                     ...
                     int *pool, double *dd_pool)
{
    ...
    int smid = get_smid();
    int *bas_kl_idx = pool + smid * QUEUE_DEPTH;
    double *dd_cache = dd_pool + smid * 1296 + sq_id;
    __shared__ int ntasks;
    if (sq_id == 0 && gout_id == 0) {
        ntasks = 0;
    }
...
int rys_ejk_ip2_type12_unrolled_o0(RysIntEnvVars *envs, JKEnergy *jk, BoundsInfo *bounds,
                        int *pool, double *dd_pool)
{
    int li = bounds->li;
    int lj = bounds->lj;
    ...
    switch (ijkl) {
    case 0:
        rys_ejk_ip2_type12_0000<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    case 125:
        rys_ejk_ip2_type12_1000<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    case 130:
        rys_ejk_ip2_type12_1010<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    case 131:
        ...
        rys_ejk_ip2_type12_1011<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    case 150:
        rys_ejk_ip2_type12_1100<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    case 155:
        buflen += (bounds->g_size * 3 + 9) * nsq_per_block;
        cudaFuncSetAttribute(rys_ejk_ip2_type12_1110, cudaFuncAttributeMaxDynamicSharedMemorySize, buflen*sizeof(double));
        rys_ejk_ip2_type12_1110<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    case 156:
        buflen += (bounds->g_size * 3 + 9) * nsq_per_block;
        cudaFuncSetAttribute(rys_ejk_ip2_type12_1111, cudaFuncAttributeMaxDynamicSharedMemorySize, buflen*sizeof(double));
        rys_ejk_ip2_type12_1111<<<npairs_ij, threads, buflen*sizeof(double)>>>(*envs, *jk, *bounds, pool, dd_pool); break;
    default: return 0;
    }
    return 1;
}
