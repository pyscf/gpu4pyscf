...
def krylov_batch(aop, b, x0=None, tol=1e-10, max_cycle=30, dot=cupy.dot,
    ...
    xs = [[] for _ in range(nroots)]
    ax = [[] for _ in range(nroots)]
    axt = cupy.zeros_like(x1)
    # skip the iterations if converged for some variables
    conv_list = np.array([False]*nroots)
    for cycle in range(max_cycle):
        x1_mask = x1[~conv_list]
        axt_mask = aop(x1_mask)
        if axt_mask.ndim == 1:
    ...
    x = cupy.empty_like(b).reshape([nroots,-1])
    for r in range(nroots):
        ...
        ax_r = ax[r]
        for i in range(nd):
            xi = cupy.asarray(xs_r[i])
            if hermi:
                for j in range(i+1):
                    h[i,j] = dot(xi.conj(), ax_r[j])
                    h[j,i] = h[i,j].conj()
            else:
                for j in range(nd):
                    h[i,j] = dot(xi.conj(), ax_r[j])
        ...
        x[r] = _gen_x0(c, cupy.asarray(xs[r]))
        if x0 is not None:
            x[r] += x0[r]
