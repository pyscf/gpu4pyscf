...
#define KERNEL_SETUP()                                                  \
  ...
  int &cell_j   = *sycl::ext::oneapi::group_local_memory<int>(thread_block); \
  int &ish_cell0= *sycl::ext::oneapi::group_local_memory<int>(thread_block); \
  int &jsh_cell0= *sycl::ext::oneapi::group_local_memory<int>(thread_block); \
  int &i0       = *sycl::ext::oneapi::group_local_memory<int>(thread_block); \
  int &j0       = *sycl::ext::oneapi::group_local_memory<int>(thread_block); \
...
#define LAUNCH_KERNEL(KERNEL)                                           \
    {                                                                   \
      ...
      sycl_get_queue()->submit([&](sycl::handler &cgh) {                \
        ...
        cgh.parallel_for<class KERNEL##_sycl>(sycl::nd_range<2>(blocks * cuda_threads, cuda_threads), [=](auto item) { \
