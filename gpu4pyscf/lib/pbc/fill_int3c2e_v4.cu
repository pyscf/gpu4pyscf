                                 ...
                                 float *atom_aux_exps, float log_cutoff)
{
    ...
    for (int kidx = kidx0+aux_id; kidx < kidx1+aux_id; kidx += aux_per_block) {
        ...
        _filter_jk_images(img_counts, img_pool, envs, bas_ij,
        ...
        if (kidx < kidx1 && to_sph && (li > 1 || lj > 1)) {
            ...
            for (int k = gout_id*nimgs_per_block+img_id; k < nfk; k += _stride) {
                for (int i = 0; i < nfi; i++) {
                    ...
                    for (int j = 0; j < dj; j++) {
                        double *sph_out = j3c + j * di * naux + k * nksh;
                        double s = 0;
                        // cart2sph for i
                        switch (lj*lj+j) {
                        case 0: { // l=0, m=0
...
extern "C" {
int PBCsr_int3c2e_latsum23a(double *out, PBCIntEnvVars *envs, uint32_t *pool,
...
int bvk_ovlp_img_idxa(int *img_idx, uint32_t *img_offsets, uint32_t *bas_ij_idx, int npairs,
                     PBCIntEnvVars *envs, float *exps, float *log_coef, float log_cutoff)
{
    constexpr int threads = 512;
