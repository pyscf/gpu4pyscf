...
#pragma unroll
    ...
    if (Gv_id < nGv) {
        ...
        int nfj = (lj + 1) * (lj + 2) / 2;
        int nfij = nfi * nfj;
        double *aft_tensor = out + (nfij * pair_ij * nGv + Gv_id) * OF_COMPLEX;
        for (int n = 0; n < GOUT_WIDTH; ++n) {
            int ij = n*gout_stride + gout_id;
            if (ij >= nfij) break;
            aft_tensor[n*2  ] = goutR[n];
            aft_tensor[n*2+1] = goutI[n];
        }
    }
}
...
//__global__
//void ft_aopair_unpack(double *out, int *conj_mapping, int bvk_ncells, int nGv)
//{
//}
