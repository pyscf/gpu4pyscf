...
int PBCcontract_int3c2e_dm(double *out, double *dm,
                           ...
                           int nbas_aux, int nbatches_shl_pair, uint32_t *bas_ij_idx,
                           int *shl_pair_offsets, int *gout_stride_lookup,
                           ...
                           float *diffuse_exps, float *diffuse_coefs, float log_cutoff)
{
    ...
    contract_int3c2e_dm_kernel<<<blocks, THREADS, shm_size>>>(
            out, dm, *envs, pool, nbatches_shl_pair, bas_ij_idx,
            shl_pair_offsets, gout_stride_lookup, img_idx, img_offsets,
            diffuse_exps, diffuse_coefs, log_cutoff);
