...
#pragma unroll
        ...
        for (int c_index = c_start+tx; c_index <= c_stop; c_index += tile) {
            ...
            for (int a_index = a_center; a_index <= a_stop; a_index++,
                ...
                double y_yi = y - yi;
                double z_zi = z - zi;
                gto_cartesian<LI>(i_cartesian, x_xi, y_yi, z_zi);
                gto_deriv1<LI>(i_gradient, i_cartesian, x_xi, y_yi, z_zi, ai);
                ...
                gto_cartesian<LJ>(j_cartesian, x_xj, y_yj, z_zj);
                gto_deriv1<LJ>(j_gradient, j_cartesian, x_xj, y_yj, z_zj, aj);
    ...
    case (li * LMAX1 + lj): \
        ...
        (sycl::nd_range<2>(grids * threads, threads), [=](auto item) [[intel::kernel_args_restrict]] { \
            eval_mgga_mat_kernel_v2<li,lj,slice_i,slice_j>( \
                out, vxc, tau, dev_envs, bas_ij_idx, grid_frac_ranges, \
