                           ...
                           double factor, double negligible)
{
    ...
    auto &xjxi = *sycl::ext::oneapi::group_local_memory_for_overwrite<double>(item.get_group());
    auto &yjyi = *sycl::ext::oneapi::group_local_memory_for_overwrite<double>(item.get_group());
    auto &zjzi = *sycl::ext::oneapi::group_local_memory_for_overwrite<double>(item.get_group());
    auto &dm_cache = *sycl::ext::oneapi::group_local_memory_for_overwrite<double[nfi*nfj]>(item.get_group());
#else
...
#define eval_mgga_grad_kernel_case(li, lj, slice_i, slice_j) \
    case (li * LMAX1 + lj): \
        sycl_get_queue()->parallel_for<class CONCAT(eval_mgga_grad_kernel_mgv3_sycl_, CONCAT(li, _##lj))> \
        (sycl::nd_range<2>(grids * threads, threads), [=](auto item) [[intel::kernel_args_restrict]] { \
            eval_mgga_grad_kernel<li,lj,slice_i,slice_j>( \
                grad, strain, dm, vxc, tau, dev_envs, bas_ij_idx, grid_frac_ranges, \
