...
__global__ static
void eval_mgga_mat_kernel(double *out, double *vxc_weights, PBCIntEnvVars envs,
                         ...
                         int mesh_a, int mesh_b, int mesh_c)
{
    ...
    __shared__ double vrho_cache[TILE*TILE*TILE];
    __shared__ double vtau_cache[TILE*TILE*TILE];
    double *vtau_weights = vxc_weights + mesh_a * mesh_b * mesh_c;
...
for (int tile_id = tile_id0; tile_id < min(tile_id0+tiles_per_block, ntiles); tile_id++) {
    ...
    if (a_idx < mesh_a && b_idx < mesh_b && c_idx < mesh_c) {
        int abc_idx = (a_idx * mesh_b + b_idx) * mesh_c + c_idx;
        vrho_cache[thread_id] = vxc_weights[abc_idx];
        vtau_cache[thread_id] = vtau_weights[abc_idx];
    }
    __syncthreads();
                ...
                for (int b_index = 0; b_index < b_upper; b_index++,
                    ...
                    for (int c_index = 0; c_index < c_upper; c_index++,
                        ...
                        if (fabs(gaussian_xyz) > 1e-18) {
                            ...
                            double rho_fac = gaussian_xyz * vrho_cache[abc_index];
                            double tau_fac = gaussian_xyz * vtau_cache[abc_index];
#pragma unroll
...
int evaluate_mgga_mat(double *out, double *vxc_weights, PBCIntEnvVars *envs,
                     ...
                     int *grid_tile_index, int n_contributing_tiles, int *mesh)
{
    ...
    switch (i_angular * LMAX1 + j_angular) {
        ...
        eval_mgga_mat_kernel_case(4,2, 8, 6, 0);
