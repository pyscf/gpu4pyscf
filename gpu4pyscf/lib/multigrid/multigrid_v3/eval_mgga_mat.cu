...
__global__ static
void eval_mgga_mat_kernel(double *out, double *vrho_weights, double *vtau_weights,
                          PBCIntEnvVars envs, double *supmol_img_coords,
...
for (int tile_id = tile_id0; tile_id < min(tile_id0+tiles_per_block, ntiles); tile_id++) {
    ...
    if (a_idx < mesh_a && b_idx < mesh_b && c_idx < mesh_c) {
        ...
        vrho_cache[thread_id] = vrho_weights[abc_idx];
        vtau_cache[thread_id] = vtau_weights[abc_idx] / 2;
    }
    __syncthreads();
                ...
                for (int b_index = 0; b_index < b_upper; b_index++,
                    ...
                    for (int c_index = 0; c_index < c_upper; c_index++,
                        ...
                        if (fabs(gaussian_xyz) > negligible) {
                            ...
                            double tau_fac = gaussian_xyz * vtau_cache[abc_index] / 2;
#pragma unroll
...
int evaluate_mgga_mat(double *out, double *vxc, double *vtau, PBCIntEnvVars *envs,
                     ...
                     double negligible)
{
    ...
    switch (i_angular * LMAX1 + j_angular) {
        ...
        eval_mgga_mat_kernel_case(3,2, 5, 6, 1);
        eval_mgga_mat_kernel_case(3,3,10, 5, 1);
