...
__global__ static void evaluate_density_kernel(
  ...
  const int thread_id = threadIdx_x + threadIdx_y * BLOCK_DIM_XYZ +
                        threadIdx_z * BLOCK_DIM_XYZ * BLOCK_DIM_XYZ;
...
#pragma unroll
  ...
  const int a_index = a_start + threadIdx_z;
  const int b_index = b_start + threadIdx_y;
  const int c_index = c_start + threadIdx_x;
...
#ifdef USE_SYCL
namespace { struct evaluate_density_tu_tag {}; }
namespace { template<int> struct evaluate_density_callsite_tag {}; }
template<int LI, int LJ, typename TUtag, typename CallTag, typename KernelTypeTag, typename NChannelsTag, typename NonOrthTag> struct evaluate_density_kernel_sycl_name;
template<int LI, int LJ, int CS, typename KernelType, int NCH, bool NONORTH>
using evaluate_density_kernel_sycl_t = evaluate_density_kernel_sycl_name<LI, LJ,
                                                                         evaluate_density_tu_tag,
                                                                         evaluate_density_callsite_tag<CS>,
...
__global__ static void evaluate_density_tau_kernel(
  ...
  auto &reduced_density_tau_values = *sycl::ext::oneapi::group_local_memory_for_overwrite<KernelType[n_channels * 2 * n_threads]>(item.get_group());
#else
  int threadIdx_x = threadIdx.x;
  int threadIdx_y = threadIdx.y;
  int threadIdx_z = threadIdx.z;
  ...
  __shared__ KernelType reduced_density_tau_values[n_channels * 2 * n_threads];
#endif
  constexpr int n_i_cartesian_functions = (i_angular + 1) * (i_angular + 2) / 2;
  constexpr int n_j_cartesian_functions = (j_angular + 1) * (j_angular + 2) / 2;
...
#pragma unroll
  for (int i_channel = 0; i_channel < n_channels * 2; i_channel++) {
    reduced_density_tau_values[i_channel * n_threads + thread_id] = 0;
  }
...
#pragma unroll
            ...
            #ifdef USE_SYCL
            const KernelType reduced_density = sycl::reduce_over_group(item.get_group(), density_value_to_be_shared, sycl::plus<>());
            #else
            ...
            #ifdef USE_SYCL
            const KernelType reduced_tau = sycl::reduce_over_group(item.get_group(), tau_value_to_be_shared, sycl::plus<>());
            #else
...
#ifdef USE_SYCL
namespace { struct evaluate_density_tau_tu_tag {}; }
namespace { template<int> struct evaluate_density_tau_callsite_tag {}; }
template<int LI, int LJ, typename TUtag, typename CallTag, typename KernelTypeTag, typename NChannelsTag, typename NonOrthTag> struct evaluate_density_tau_kernel_sycl_name;
template<int LI, int LJ, int CS, typename KernelType, int NCH, bool NONORTH>
using evaluate_density_tau_kernel_sycl_t = evaluate_density_tau_kernel_sycl_name<LI, LJ,
                                                                         evaluate_density_tau_tu_tag,
                                                                         evaluate_density_tau_callsite_tag<CS>,
                                                                         KernelType,                                 // encodes KernelType
                                                                         std::integral_constant<int, NCH>,          // encodes n_channels
                                                                         std::bool_constant<NONORTH>>;                // encodes is_non_orthogonal
...
#else
#define density_tau_kernel_macro(li, lj)                                       \
  evaluate_density_tau_kernel<KernelType, n_channels, li, lj,                  \
...
#define density_tau_kernel_case_macro(li, lj)                                  \
  case (li * 10 + lj):                                                         \
    density_tau_kernel_macro(li, lj);                                          \
    break
...
int evaluate_density_tau_driver(
  ...
  switch (i_angular * 10 + j_angular) {
    ...
    density_tau_kernel_case_macro(0, 2);
    density_tau_kernel_case_macro(0, 3);
    density_tau_kernel_case_macro(0, 4);
    density_tau_kernel_case_macro(1, 0);
    density_tau_kernel_case_macro(1, 1);
    density_tau_kernel_case_macro(1, 2);
    density_tau_kernel_case_macro(1, 3);
    density_tau_kernel_case_macro(1, 4);
    density_tau_kernel_case_macro(2, 0);
    density_tau_kernel_case_macro(2, 1);
    density_tau_kernel_case_macro(2, 2);
    density_tau_kernel_case_macro(2, 3);
    density_tau_kernel_case_macro(2, 4);
    density_tau_kernel_case_macro(3, 0);
    density_tau_kernel_case_macro(3, 1);
    density_tau_kernel_case_macro(3, 2);
    density_tau_kernel_case_macro(3, 3);
    density_tau_kernel_case_macro(3, 4);
    density_tau_kernel_case_macro(4, 0);
    density_tau_kernel_case_macro(4, 1);
    density_tau_kernel_case_macro(4, 2);
    density_tau_kernel_case_macro(4, 3);
    density_tau_kernel_case_macro(4, 4);
  default:
    fprintf(stderr,
            "angular momentum pair %d, %d is not supported in "
            "evaluate_density_tau_driver\n",
            i_angular, j_angular);
    return 1;
  }
...
#define xc_with_tau_kernel_case_macro(li, lj)                                  \
  case (li * 10 + lj):                                                         \
    xc_with_tau_kernel_macro(li, lj);                                          \
    break
...
int evaluate_xc_with_tau_driver(
  ...
  switch (i_angular * 10 + j_angular) {
    ...
    xc_with_tau_kernel_case_macro(0, 1);
    xc_with_tau_kernel_case_macro(0, 2);
    xc_with_tau_kernel_case_macro(0, 3);
    xc_with_tau_kernel_case_macro(0, 4);
    xc_with_tau_kernel_case_macro(1, 0);
    xc_with_tau_kernel_case_macro(1, 1);
    xc_with_tau_kernel_case_macro(1, 2);
    xc_with_tau_kernel_case_macro(1, 3);
    xc_with_tau_kernel_case_macro(1, 4);
    xc_with_tau_kernel_case_macro(2, 0);
    xc_with_tau_kernel_case_macro(2, 1);
    xc_with_tau_kernel_case_macro(2, 2);
    xc_with_tau_kernel_case_macro(2, 3);
    xc_with_tau_kernel_case_macro(2, 4);
    xc_with_tau_kernel_case_macro(3, 0);
    xc_with_tau_kernel_case_macro(3, 1);
    xc_with_tau_kernel_case_macro(3, 2);
    xc_with_tau_kernel_case_macro(3, 3);
    xc_with_tau_kernel_case_macro(3, 4);
    xc_with_tau_kernel_case_macro(4, 0);
    xc_with_tau_kernel_case_macro(4, 1);
    xc_with_tau_kernel_case_macro(4, 2);
    xc_with_tau_kernel_case_macro(4, 3);
    xc_with_tau_kernel_case_macro(4, 4);
  default:
    fprintf(stderr,
            "angular momentum pair %d, %d is not supported in "
