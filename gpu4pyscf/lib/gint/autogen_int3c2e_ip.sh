...
template <> __global__
void GINTfill_int3c2e_${IP_TYPE}_kernel<${ROOT},${GS}>(GINTEnvVars envs, ERITensor eri, BasisProdOffsets offsets)
{
    ${BLK0}
    ...
    for (ij = prim_ij; ij < prim_ij+nprim_ij; ++ij) {
        for (kl = prim_kl; kl < prim_kl+nprim_kl; ++kl) {
            ...
            ${NABLA_SCRIPT}
            GINTwrite_int3c2e_ip_direct<${ROOT}>(envs, eri, f, g, ish, jsh, ksh);
    } }
}
#endif " >> ${FILE}
    done
