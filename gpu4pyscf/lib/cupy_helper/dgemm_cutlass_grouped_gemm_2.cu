...
// A100
using cutlass_tensorop_d884gemm_grouped_128x64_16x3_tt_align1_base =
...
// V100
using cutlass_simt_dgemm_grouped_128x64_8x2_tt_align1_base =
  typename cutlass::gemm::kernel::DefaultGemmGrouped<
    double, cutlass::layout::ColumnMajor, cutlass::ComplexTransform::kNone, 1,
    double, cutlass::layout::RowMajor, cutlass::ComplexTransform::kNone, 1,
    double, cutlass::layout::RowMajor,
...
int grouped_dgemm_2(cudaStream_t stream, uint64_t *out, uint64_t *x, uint64_t *y, int64_t *Ms, int64_t *Ns, int64_t *Ks, int num)
{
    int compute_capability = get_device_compute_capability();

    if(compute_capability == 70)
    {
      using DeviceKernel = cutlass::gemm::device::GemmGrouped<cutlass_simt_dgemm_grouped_128x64_8x2_tt_align1_base>;
      grouped_gemm_kernel_launch<DeviceKernel>(out, x, y, Ms, Ns, Ks, num);
    }
    else if(compute_capability == 80)
    {
      using DeviceKernel = cutlass::gemm::device::GemmGrouped<cutlass_tensorop_d884gemm_grouped_128x64_16x3_tt_align1_base>;
      grouped_gemm_kernel_launch<DeviceKernel>(out, x, y, Ms, Ns, Ks, num);
    }
    else
    {
