...
__global__
#endif
void md_j_0_0_s4(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds)
{
    ...
    double *vj_kl_cache = Rq_cache + 2048;
...
__global__
#endif
void md_j_1_1_s4(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds)
{
    int *pair_ij_mapping = bounds.tile_ij_mapping;
    int *pair_kl_mapping = bounds.tile_kl_mapping;
    int task_ij0 = blockIdx.x * 512;
    ...
    double *Rq_cache = Rp_cache + 64;
    double *vj_kl_cache = Rq_cache + 1088;
    double *dm_ij_cache = vj_kl_cache + 1088;
    float *qd_ij_max = bounds.qd_ij_max;
    float *qd_kl_max = bounds.qd_kl_max;
    // zero out all cache;
...
__global__
#endif
void md_j_2_2_s4(RysIntEnvVars envs, JKMatrix jk, BoundsInfo bounds)
{
    int *pair_ij_mapping = bounds.tile_ij_mapping;
    int *pair_kl_mapping = bounds.tile_kl_mapping;
    int task_ij0 = blockIdx.x * 512;
    ...
    double *Rq_cache = Rp_cache + 64;
    double *vj_kl_cache = Rq_cache + 448;
    double *dm_ij_cache = vj_kl_cache + 1120;
    float *qd_ij_max = bounds.qd_ij_max;
    float *qd_kl_max = bounds.qd_kl_max;
    // zero out all cache;
    for (int n = sq_id; n < 512; n += 256) {
...
int md_j_unrolled(RysIntEnvVars *envs, JKMatrix *jk, BoundsInfo *bounds)
{
    ...
    int npairs_ij = bounds->npairs_ij;
    int npairs_kl = bounds->npairs_kl;
    if (li == lk && lj == ll) { // diagonal patterns
        switch (ijkl) {
        case 0: { // lij=0, lkl=0, tilex=32, tiley=32
            dim3 threads(16, 16);
            dim3 blocks((npairs_ij + 511) / 512, (npairs_kl + 511) / 512);
            md_j_0_0_s4<<<blocks, threads, 2896*sizeof(double)>>>(*envs, *jk, *bounds);
        } break;
        case 10: { // lij=1, lkl=1, tilex=32, tiley=17
            dim3 threads(16, 16);
            dim3 blocks((npairs_ij + 511) / 512, (npairs_kl + 271) / 272);
            md_j_1_1_s4<<<blocks, threads, 3072*sizeof(double)>>>(*envs, *jk, *bounds);
        } break;
        case 20: { // lij=2, lkl=2, tilex=32, tiley=7
            dim3 threads(16, 16);
            dim3 blocks((npairs_ij + 511) / 512, (npairs_kl + 111) / 112);
            md_j_2_2_s4<<<blocks, threads, 3072*sizeof(double)>>>(*envs, *jk, *bounds);
        } break;
        default: return 0;
        }
        return 1;
    ...
    switch (ijkl) {
    case 0: { // lij=0, lkl=0, tilex=32, tiley=25
        dim3 threads(16, 16);
    ...
    case 10: { // lij=1, lkl=1, tilex=32, tiley=9
        dim3 threads(16, 16);
    ...
    case 20: { // lij=2, lkl=2, tilex=32, tiley=11
        dim3 threads(16, 16);
