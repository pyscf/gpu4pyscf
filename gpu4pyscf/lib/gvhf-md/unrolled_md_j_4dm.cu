...
void md_j_4dm_0_0(RysIntEnvVars envs, JKMatrix jk, MDBoundsInfo bounds, int dm_size)
{
    ...
    if (pair_ij_mapping == pair_kl_mapping && task_ij0+464 <= task_kl0) {
        return;
    }
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    double *Rq_cache = vj_kl_cache + 3712;
...
while (remaining_n_dm > 0) {
    for (int batch_ij = 0; batch_ij < 29; ++batch_ij) {
        ...
        for (int batch_kl = 0; batch_kl < 29; ++batch_kl) {
            ...
            int pair_ij0 = pair_ij_mapping[task_ij0];
            int pair_kl0 = pair_kl_mapping[task_kl0];
            if (qd_ij_max[batch_ij+blockIdx.x*29] + q_cond[pair_kl0] < bounds.cutoff &&
...
#pragma unroll
            for (int n = 0; n < 1; ++n) {
                ...
                for (int stride = 8.0; stride > 0; stride /= 2) {
                    __syncthreads();
                    if (ty < stride) {
...
#pragma unroll
            for (int n = 0; n < 1; ++n) {
                ...
                for (int stride = 8.0; stride > 0; stride /= 2) {
                    ...
                    if (ty < stride) {
                        vj_cache[thread_id] += vj_cache [thread_id + stride*16];
                        vj_cache[thread_id] += vj_cache1[thread_id + stride*16];
                    }
                }
                __syncthreads();
                if (ty == 0 && task_ij0+tx < npairs_ij) {
...
while (remaining_n_dm > 0) {
    for (int batch_ij = 0; batch_ij < 48; ++batch_ij) {
        ...
        for (int batch_kl = 0; batch_kl < 26; ++batch_kl) {
            ...
            if (tx == 0) {
            for (int m = 0; m < 8; ++m) {
                vj_kl_cache[sq_kl+m*416+0] += vj_kl[m];
            } }
            for (int m = 0; m < 8; ++m) { dm_kl[m] = 1 * dm[m*dm_size+kl_loc0+0]; }
            vj_ij[0] += gamma_inc[sq_id+0*256] * dm_kl[0];
...
void md_j_4dm_1_1(RysIntEnvVars envs, JKMatrix jk, MDBoundsInfo bounds, int dm_size)
{
    ...
    if (pair_ij_mapping == pair_kl_mapping && task_ij0+128 <= task_kl0) {
        return;
    }
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    int npairs_kl = bounds.npairs_kl;
    extern __shared__ double vj_kl_cache[];
    double *Rq_cache = vj_kl_cache + 4096;
    double *Rp_cache = Rq_cache + 512;
    double *dm_ij_cache = Rp_cache + 64;
    double *gamma_inc = dm_ij_cache + 512;
    float *qd_ij_max = bounds.qd_ij_max;
    float *qd_kl_max = bounds.qd_kl_max;
    ...
    for (int n = thread_id; n < 128; n += 256) {
...
while (remaining_n_dm > 0) {
    for (int batch_ij = 0; batch_ij < 8; ++batch_ij) {
        ...
        for (int batch_kl = 0; batch_kl < 8; ++batch_kl) {
            ...
            int pair_ij0 = pair_ij_mapping[task_ij0];
            int pair_kl0 = pair_kl_mapping[task_kl0];
            if (qd_ij_max[batch_ij+blockIdx.x*8] + q_cond[pair_kl0] < bounds.cutoff &&
                qd_kl_max[batch_kl+blockIdx.y*8] + q_cond[pair_ij0] < bounds.cutoff) {
                continue;
            }
            ...
            if (tx == 0) {
            for (int m = 0; m < 8; ++m) {
                vj_kl_cache[sq_kl+m*512+0] += vj_kl[m];
            } }
            for (int m = 0; m < 8; ++m) vj_kl[m] = 0.;
            ...
            if (tx == 0) {
            for (int m = 0; m < 8; ++m) {
                vj_kl_cache[sq_kl+m*512+128] += vj_kl[m];
            } }
            for (int m = 0; m < 8; ++m) vj_kl[m] = 0.;
            ...
            if (tx == 0) {
            for (int m = 0; m < 8; ++m) {
                vj_kl_cache[sq_kl+m*512+256] += vj_kl[m];
            } }
            for (int m = 0; m < 8; ++m) vj_kl[m] = 0.;
            ...
            if (tx == 0) {
            for (int m = 0; m < 8; ++m) {
                vj_kl_cache[sq_kl+m*512+384] += vj_kl[m];
            } }
            for (int m = 0; m < 8; ++m) { dm_kl[m] = 1 * dm[m*dm_size+kl_loc0+0]; }
            vj_ij[0] += gamma_inc[sq_id+0*256] * dm_kl[0];
            vj_ij[4] += gamma_inc[sq_id+0*256] * dm_kl[1];
...
int md_j_4dm_unrolled(RysIntEnvVars *envs, JKMatrix *jk, MDBoundsInfo *bounds, int dm_size)
{
    ...
    int npairs_kl = bounds->npairs_kl;
    switch (ijkl) {
    case 0: { // lij=0, lkl=0, tilex=29, tiley=29
        dim3 threads(16, 16);
        dim3 blocks((npairs_ij + 463) / 464, (npairs_kl + 463) / 464, 1);
        md_j_4dm_0_0<<<blocks, threads, 6016*sizeof(double)>>>(*envs, *jk, *bounds, dm_size);
    } break;
    case 9: { // lij=1, lkl=0, tilex=48, tiley=26
        dim3 threads(16, 16);
        dim3 blocks((npairs_ij + 767) / 768, (npairs_kl + 415) / 416, 1);
        md_j_4dm_1_0<<<blocks, threads, 6080*sizeof(double)>>>(*envs, *jk, *bounds, dm_size);
    } break;
    case 10: { // lij=1, lkl=1, tilex=8, tiley=8
        dim3 threads(16, 16);
        dim3 blocks((npairs_ij + 127) / 128, (npairs_kl + 127) / 128, 1);
        md_j_4dm_1_1<<<blocks, threads, 5952*sizeof(double)>>>(*envs, *jk, *bounds, dm_size);
    } break;
