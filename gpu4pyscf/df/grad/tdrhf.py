...
from gpu4pyscf.df.grad.jk import get_rhojk_dm_only, get_grad_vjk_dmonly
...
def _decompose_rdm1_svd(dm):
    ...
    idx = cp.abs(s)>1e-8
    return cp.asfortranarray(u[:,idx]), cp.asfortranarray(contract('i,ip->pi', s[idx], vh[idx]))
...
def get_jk(tdgrad, mol=None, dm0=None, hermi=0, with_j=True, with_k=True, omega=None):
    ...
    rhoj, rhok = get_rhojk_dm_only(with_df, dm, orbol, orbor, with_j=with_j, with_k=with_k)
    ...
    vj, vk, vjaux, vkaux = get_grad_vjk_dmonly(with_df, mol, auxmol, rhoj_cart, dm_cart, rhok_cart, orbol_cart, orbor_cart,
