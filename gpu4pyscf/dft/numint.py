...
def eval_ao(ni, mol, coords, deriv=0, shls_slice=None, nao_slice=None, ao_loc_slice=None,
    ...
    if shls_slice is None:
        shls_slice = cupy.arange(_mol.nbas, dtype=np.int32)
        ctr_offsets = opt.l_ctr_offsets
        ctr_offsets_slice = opt.l_ctr_offsets
        ao_loc_slice = cupy.asarray(_mol.ao_loc_nr())
        nao_slice = _mol.nao
    else:
        ctr_offsets = opt.l_ctr_offsets
    ...
    if not with_opt:
        # mol may be different to _GDFTOpt._mol.
        # nao should be consistent with the _GDFTOpt._mol object
        coeff = cupy.asarray(opt.coeff)
        with opt.gdft_envs_cache():
            err = libgdft.GDFTeval_gto(
                ctypes.cast(stream.ptr, ctypes.c_void_p),
                ctypes.cast(out.data.ptr, ctypes.c_void_p),
                ctypes.c_int(deriv), ctypes.c_int(_mol.cart),
                ...
                _mol._bas.ctypes.data_as(ctypes.c_void_p))
...
def nr_rks(ni, mol, grids, xc_code, dms, relativity=0, hermi=1,
    ...
    for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv):
        p1 = p0 + weight.size
        for i in range(nset):
            if mo_coeff is None:
                rho_tot[i,:,p0:p1] = eval_rho(_mol, ao_mask, dms[i][np.ix_(idx,idx)], xctype=xctype, hermi=1, with_lapl=with_lapl)
            else:
                mo_coeff_mask = mo_coeff[idx,:]
                rho_tot[i,:,p0:p1] = eval_rho2(_mol, ao_mask, mo_coeff_mask, mo_occ, None, xctype, with_lapl)
        p0 = p1
...
def nr_rks_group(ni, mol, grids, xc_code, dms, relativity=0, hermi=1,
    ...
    t1 = t0
    p0 = p1 = 0
    for ao_mask_group, idx_group, weight_group, _ in ni.grouped_block_loop(_mol, grids, nao, ao_deriv):
        p0_raw = p0
        for i in range(nset):
            p0 = p0_raw
            if xctype == 'LDA':
...
def nr_uks(ni, mol, grids, xc_code, dms, relativity=0, hermi=1,
    ...
    for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv):
        for i in range(nset):
            t0 = log.init_timer()
            if mo_coeff is None:
                rho_a = eval_rho(_mol, ao_mask, dma[i][np.ix_(idx,idx)], xctype=xctype, hermi=1, with_lapl=with_lapl)
                rho_b = eval_rho(_mol, ao_mask, dmb[i][np.ix_(idx,idx)], xctype=xctype, hermi=1, with_lapl=with_lapl)
            else:
                mo_coeff_mask = mo_coeff[:, idx,:]
                rho_a = eval_rho2(_mol, ao_mask, mo_coeff_mask[0], mo_occ[0], None, xctype, with_lapl)
                rho_b = eval_rho2(_mol, ao_mask, mo_coeff_mask[1], mo_occ[1], None, xctype, with_lapl)
...
def get_rho(ni, mol, dm, grids, max_memory=2000, verbose=None):
    ...
    p0 = p1 = 0
    t1 = t0 = log.init_timer()
    for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, 0):
        p1 = p0 + weight.size
        if mo_coeff is None:
            rho[p0:p1] = eval_rho(_mol, ao_mask, dm[np.ix_(idx,idx)], xctype='LDA', hermi=1, with_lapl=with_lapl)
        else:
            mo_coeff_mask = mo_coeff[idx,:]
            rho[p0:p1] = eval_rho2(_mol, ao_mask, mo_coeff_mask, mo_occ, None, 'LDA', with_lapl)
        p0 = p1
        t1 = log.timer_debug2('eval rho slice', *t1)
    t0 = log.timer_debug1('eval rho', *t0)
...
def nr_rks_fxc(ni, mol, grids, xc_code, dm0=None, dms=None, relativity=0, hermi=0,
    ...
    p1 = 0
    t1 = t0 = log.init_timer()
    for ao, mask, weights, coords in ni.block_loop(_mol, grids, nao, ao_deriv):
        p0, p1 = p1, p1+len(weights)
        # precompute molecular orbitals
        if with_mocc:
            occ_coeff_mask = occ_coeff[mask]
            if xctype == 'LDA':
                c0 = _dot_ao_dm(_mol, ao, occ_coeff_mask, None, None, None)
            elif xctype == "GGA":
        ...
        if with_mocc:
            rho1 = eval_rho4(_mol, ao, c0, mo1[:,mask], xctype=xctype, with_lapl=False)
        else:
            # slow version
            rho1 = []
            for i in range(nset):
                rho_tmp = eval_rho(_mol, ao, dms[i][np.ix_(mask,mask)], xctype=xctype, hermi=hermi, with_lapl=with_lapl)
                rho1.append(rho_tmp)
...
def nr_uks_fxc(ni, mol, grids, xc_code, dm0=None, dms=None, relativity=0, hermi=0,
    ...
    for ao, mask, weights, coords in ni.block_loop(_mol, grids, nao, ao_deriv):
        ...
        if with_mocc:
            ...
            if xctype == 'LDA':
                c0_a = _dot_ao_dm(_mol, ao, occ_coeff_a_mask, None, None, None)
                c0_b = _dot_ao_dm(_mol, ao, occ_coeff_b_mask, None, None, None)
            elif xctype == "GGA":
        ...
        if with_mocc:
            rho1a = eval_rho4(_mol, ao, c0_a, mo1a[:,mask], xctype=xctype, with_lapl=with_lapl)
            rho1b = eval_rho4(_mol, ao, c0_b, mo1b[:,mask], xctype=xctype, with_lapl=with_lapl)
        else:
            # slow version
            rho1a = []
            rho1b = []
            for i in range(nset):
                rho_tmp = eval_rho(_mol, ao, dma[i][np.ix_(mask,mask)], xctype=xctype, hermi=hermi, with_lapl=with_lapl)
                rho1a.append(rho_tmp)
                rho_tmp = eval_rho(_mol, ao, dmb[i][np.ix_(mask,mask)], xctype=xctype, hermi=hermi, with_lapl=with_lapl)
...
def nr_nlc_vxc(ni, mol, grids, xc_code, dms, relativity=0, hermi=1,
    ...
    for ao, idx, weight, coords \
            in ni.block_loop(_mol, grids, nao, ao_deriv, max_memory=max_memory):
        #rho = eval_rho(opt.mol, ao, dms[0][np.ix_(mask,mask)], xctype='GGA', hermi=1)
        if mo_coeff is None:
            rho = eval_rho(_mol, ao, dms[0][np.ix_(idx,idx)], xctype='GGA', hermi=1, with_lapl=with_lapl)
        else:
            mo_coeff_mask = mo_coeff[idx,:]
            rho = eval_rho2(_mol, ao, mo_coeff_mask, mo_occ, None, 'GGA', with_lapl)
        vvrho.append(rho)
...
def cache_xc_kernel(ni, mol, grids, xc_code, mo_coeff, mo_occ, spin=0,
    ...
    if spin == 0:
        ...
        for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv):
            mo_coeff_mask = mo_coeff[idx,:]
            rho_slice = eval_rho2(_mol, ao_mask, mo_coeff_mask, mo_occ, None, xctype, with_lapl)
        ...
        for ao_mask, idx, weight, _ in ni.block_loop(_mol, grids, nao, ao_deriv):
            mo_coeff_mask = mo_coeff[:,idx,:]
            rhoa_slice = eval_rho2(_mol, ao_mask, mo_coeff_mask[0], mo_occ[0], None, xctype, with_lapl)
            rhob_slice = eval_rho2(_mol, ao_mask, mo_coeff_mask[1], mo_occ[1], None, xctype, with_lapl)
    ...
    with opt.gdft_envs_cache():
        ...
        for ip0, ip1 in lib.prange(0, ngrids, blksize):
            ...
            if (block_id, blksize, ngrids) not in ni.non0ao_idx:
                ni.non0ao_idx[block_id, blksize, ngrids] = _sparse_index(_mol, coords, opt.l_ctr_offsets)

            pad, idx, non0shl_idx, ctr_offsets_slice, ao_loc_slice = ni.non0ao_idx[block_id, blksize, ngrids]
            ao_mask = eval_ao(
                ni, _mol, coords, deriv,
                nao_slice=len(idx),
...
class _GDFTOpt:
    def __init__(self, mol):
        ...
        self._mol = None       # sorted mol object based on contraction pattern
        self.mol = mol
    ...
    def gdft_envs_cache(self):
        ...
        ao_loc = _mol.ao_loc_nr()
        libgdft.GDFTinit_envs(
            ctypes.byref(self.envs_cache), ao_loc.ctypes.data_as(ctypes.c_void_p),
            _mol._atm.ctypes.data_as(ctypes.c_void_p), ctypes.c_int(_mol.natm),
            _mol._bas.ctypes.data_as(ctypes.c_void_p), ctypes.c_int(_mol.nbas),
            _mol._env.ctypes.data_as(ctypes.c_void_p), ctypes.c_int(_mol._env.size))
        try:
            yield
        finally:
