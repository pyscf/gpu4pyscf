...
class KnownValues(unittest.TestCase):
    def test_eval_ao(self):
        ni = numint.NumInt()
        ao = ni.eval_ao(cell, grids.coords)
        ...
        ao = ni.eval_ao(cell, grids.coords, deriv=1)
    ...
    def test_eval_ao_kpt(self):
        ...
        ni = numint.NumInt()
        ao = ni.eval_ao(cell, grids.coords, kpt)
        ...
        ao = ni.eval_ao(cell, grids.coords, kpt, deriv=1)
    ...
    def test_nr_rks(self):
        ...
        dms = dms.dot(dms.T)
        ni = numint.NumInt()
        ne, exc, vmat = ni.nr_rks(cell, grids, 'm06', dms, hermi=1, kpt=kpts[0])
        ref = ni.to_cpu().nr_rks(cell, grids.to_cpu(), 'm06', dms.get(), hermi=1, kpt=kpts[0])
        ...
        ne, exc, vmat = ni.nr_rks(cell, grids, 'blyp', dms, hermi=1, kpt=kpts[0])
        ref = ni.to_cpu().nr_rks(cell, grids.to_cpu(), 'blyp', dms.get(), hermi=1, kpt=kpts[0])
        self.assertAlmostEqual(float(ne), ref[0], 9)
        self.assertAlmostEqual(float(exc), ref[1], 9)
        self.assertAlmostEqual(abs(vmat.get() - ref[2]).max(), 0, 9)

        ne, exc, vmat = ni.nr_rks(cell, grids, 'lda', dms, hermi=1, kpt=kpts[0])
    ...
    def test_nr_uks(self):
        ...
        ni = numint.NumInt()
        ne, exc, vmat = ni.nr_uks(cell, grids, 'm06', dms, hermi=1, kpt=kpts[0])
        ref = ni.to_cpu().nr_uks(cell, grids.to_cpu(), 'm06', dms.get(), hermi=1, kpt=kpts[0])
        self.assertAlmostEqual(abs(ne.get() - ref[0]).max(), 0, 9)
        self.assertAlmostEqual(float(exc), ref[1], 9)
        self.assertAlmostEqual(abs(vmat.get() - ref[2]).max(), 0, 9)

        ne, exc, vmat = ni.nr_uks(cell, grids, 'blyp', dms, hermi=1, kpt=kpts[0])
        ref = ni.to_cpu().nr_uks(cell, grids.to_cpu(), 'blyp', dms.get(), hermi=1, kpt=kpts[0])
        self.assertAlmostEqual(abs(ne.get() - ref[0]).max(), 0, 9)
        self.assertAlmostEqual(float(exc), ref[1], 9)
        self.assertAlmostEqual(abs(vmat.get() - ref[2]).max(), 0, 9)

        ne, exc, vmat = ni.nr_uks(cell, grids, 'lda', dms, hermi=1, kpt=kpts[0])
    ...
    def test_eval_rho(self):
        ...
        ao = ni.eval_ao(cell, grids.coords, deriv=0)
