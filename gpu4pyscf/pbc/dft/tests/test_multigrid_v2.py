...
class KnownValues(unittest.TestCase):
    ...
    def test_unique_image_pairs(self):
        ...
        assert abs(lib.fp(ret[0].get()) - 1.5047201402319172) < 1e-12
        assert abs(lib.fp(ret[1].get()) - -483.0210637951298) < 1e-12

        np.random.seed(2)
        Ls = Ls[np.random.rand(len(Ls)) > .5]
        ret = multigrid._unique_image_pair(Ls)
        assert ret[0].shape == (2377, 3)
        assert abs(lib.fp(ret[0].get()) - -34.99306750756055) < 1e-12
        assert abs(lib.fp(ret[1].get()) - 1347.464804553046) < 1e-12
        ...
        assert abs(lib.fp(ret[0].get()) - -1.355090495130784) < 1e-12
        assert abs(lib.fp(ret[1].get()) - 2145.771285837819) < 1e-12
