...
def get_nacv_ge(td_nac, x_yI, EI, singlet=True, atmlst=None, verbose=logger.INFO):
    ...
    if getattr(td_nac.base, 'precond_method', None) is None:
        precond_method = td_nac.precond_method
    else:
        precond_method = td_nac.base.precond_method
    if precond_method is None:
        ...
        converged, solution_vectors = tdscf._krylov_tools.krylov_solver(
                            ...
                            problem_type='linear', rhs=(LI*1.0*EI).reshape(1,nvir*nocc), # for krylov solver, rhs is -wvo
                            conv_tol=td_nac.cphf_conv_tol, max_iter=td_nac.cphf_max_cycle,
                            gram_schmidt=True, verbose=5, single=False)
        z1 = solution_vectors.reshape(nvir, nocc)
    elif precond_method.lower()[0] == 'r':
        e_a = mo_energy[mo_occ==0]
        e_i = mo_energy[mo_occ>0]
        hdiag = e_a[:,None] - e_i
        def vind_vo(mo1):
...
def get_nacv_ee(td_nac, x_yI, x_yJ, EI, EJ, singlet=True, atmlst=None, verbose=logger.INFO):
    ...
    if td_nac.base.precond_method is None:
        z1 = cphf.solve(
            fvind,
            mo_energy,
            mo_occ,
            ...
            tol=td_nac.cphf_conv_tol)[0]
        z1 = z1.reshape(nvir, nocc)
    elif td_nac.base.precond_method.lower()[0] == 'p':
        e_a = mo_energy[mo_occ==0]
        e_i = mo_energy[mo_occ>0]
        hdiag = e_a[:,None] - e_i
        def vind_vo(mo1):
        ...
        converged, solution_vectors = tdscf._krylov_tools.krylov_solver(
                            matrix_vector_product=vind_vo, hdiag=hdiag.reshape(nvir*nocc),
                            problem_type='linear', rhs=-(wvo/(EJ-EI)).reshape(1,nvir*nocc), # for krylov solver, rhs is -wvo
                            conv_tol=td_nac.cphf_conv_tol, max_iter=td_nac.cphf_max_cycle,
                            gram_schmidt=True, verbose=5, single=False)
        z1 = solution_vectors.reshape(nvir, nocc)
    elif td_nac.base.precond_method.lower()[0] == 'r':
        e_a = mo_energy[mo_occ==0]
        e_i = mo_energy[mo_occ>0]
        hdiag = e_a[:,None] - e_i
        def vind_vo(mo1):
