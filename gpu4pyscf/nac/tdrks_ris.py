...
def get_nacv_ee(td_nac, x_yI, x_yJ, EI, EJ, singlet=True, atmlst=None, verbose=logger.INFO):
    ...
    if hasattr(td_grad, 'jk_energy_per_atom'):
        ...
        if with_k and omega != 0:
            ...
            dvhf += td_grad.jk_energy_per_atom(dms, j_factor, k_factor, omega=omega, hermi=1)
    else:
    ...
    dvhf += tdrks_ris.jk_energy_per_atom(mf_J, mf_K, dms, j_factor, k_factor)
    if with_k and omega != 0:
        j_factor = None
        beta = alpha-hyb  # =beta
        k_factor = np.array([1, 1, -1, -1, 1]) * beta
        dvhf += tdrks_ris.jk_energy_per_atom(mf_J, mf_K, dms, j_factor, k_factor, omega=omega)
