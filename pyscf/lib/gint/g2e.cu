...
static void GINTfill_int2e_kernel(ERITensor eri, BasisProdOffsets offsets)
{
...
__global__
static void GINTfill_int2e_kernel0010(ERITensor eri, BasisProdOffsets offsets)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__
static void GINTfill_int2e_kernel1000(ERITensor eri, BasisProdOffsets offsets)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = blockIdx.x * blockDim.x + threadIdx.x;
...
#if POLYFIT_ORDER >= 3
template <> __global__
void GINTfill_int2e_kernel<3, GOUTSIZE3>(ERITensor eri, BasisProdOffsets offsets)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (kl = prim_kl; kl < prim_kl+nprim_kl; ++kl) {
        ...
        double x = a0 * (xijxkl * xijxkl + yijykl * yijykl + zijzkl * zijzkl);
        GINTrys_root3(x, uw);
        GINTg0_2e_2d4d<3>(g, uw, norm, as_ish, as_jsh, as_ksh, as_lsh, ij, kl);
        GINTgout2e<3>(gout, g);
    } }
...
#if POLYFIT_ORDER >= 4
template <> __global__
void GINTfill_int2e_kernel<4, GOUTSIZE4>(ERITensor eri, BasisProdOffsets offsets)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = blockIdx.x * blockDim.x + threadIdx.x;
...
#if POLYFIT_ORDER >= 5
template <> __global__
void GINTfill_int2e_kernel<5, GOUTSIZE5>(ERITensor eri, BasisProdOffsets offsets)
{
    int ntasks_ij = offsets.ntasks_ij;
    int ntasks_kl = offsets.ntasks_kl;
    int task_ij = blockIdx.x * blockDim.x + threadIdx.x;
